...
const productsContainerDiv = document.getElementById("container")
var productListHtml = ""

class Product {
    constructor(uuid, displayName,displayIcon, splash,mapUrl) {
        this.uuid = uuid
...
class ProductComponent {
    ...
    toHtml() {
        ...
        <div class="product-card" onclick="productSelected(${this.pos})">
            ...
            <div>
                <h1>${this.product.displayName}</h3>
            </div>
        </div>
        `
    }
...
function parseMapToProduct(map) {
    return new Product(map.uuid, map.mapUrl, map.displayIcon, map.displayName, map.splash)
}
